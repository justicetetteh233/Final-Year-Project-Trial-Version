...
def hbeosa(pop_size, MaxIter, trainX, trainy, testX, testy, modelrates, runfilename, metrics_result_dir, hybrid, threshold_only):
    ...
    print('evolution will start soon')

    curve, cost, featCnt, BestSol, AllSol=evolve(MaxIter, modelrates, pop_size, dimension, trainX, testX, trainy, testy, runfilename, metrics_result_dir, hybrid, threshold_only)
    return curve, cost, featCnt, BestSol, AllSol
...
def evolve(MaxIter, model_rates, pop_size, dimension, trainX, testX, trainy, testy, runfilename, metrics_result_dir, hybrid, threshold_only):
    print("ebola evolution has started")     
    incubation_period=5
    epxilon=0.0001
    SEARCHABLE_LIMIT=(pop_size*10)/100 # to swtich between random and linear infection in a population
    S, E, I, H, R, V, D, Q, PE=[], [], [], [], [], [], [], [], []
    # print("lets generated the  first face of infected humans and non humans host keep in mind we have containers to keep track of  s,e,i,h,r,v,d,q,pe and so on")
    ...
    # i added it 
    biniarySearchspaceData = S
    
    print('featurebinarization')
    print(biniarySearchspaceData)
    
    # Extract the binary arrays and fitness values from biniarySearchspaceData
    binary_arrays = [item[1][0] for item in biniarySearchspaceData]
    fitness_values = [item[1][1] for item in biniarySearchspaceData]
    
    # Create a DataFrame with binary arrays and fitness values
    featurebinarization = pd.DataFrame(binary_arrays)
    featurebinarization['Fitness'] = fitness_values
    # Display only the first 20 columns of the binary arrays and the fitness column
    display(featurebinarization.iloc[:, :20])  # First 20 binary columns
    display(featurebinarization[['Fitness']])  # Fitness column
    ...
    return allfit, allcost, featCnt, gbest[ID_INDIVIDUAL][EOSA_ID_POS], S
